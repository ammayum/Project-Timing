...
export async function ensurePalletSchema() {
  ...
  // Older draft versions prevented palletized kits from moving independently.
  // Individual movement is now allowed and automatically removes the kit from
  // pallet membership in the service transaction, so remove those guards.
  await pool.query("DROP TRIGGER IF EXISTS trg_inventory_pallet_asset_update_guard");
  await pool.query("DROP TRIGGER IF EXISTS trg_inventory_pallet_movement_item_guard");
