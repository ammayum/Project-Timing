...
const envSchema = z.object({
  ...
  PASSWORD_EXPIRY_DAYS: z.coerce.number().int().min(1).max(365).default(90),
  PASSWORD_HISTORY_COUNT: z.coerce.number().int().min(1).max(24).default(12),
  PASSWORD_MIN_AGE_DAYS: z.coerce.number().int().min(0).max(30).default(1),
  PASSWORD_MAX_FAILED_LOGINS: z.coerce.number().int().min(3).max(20).default(5),
  PASSWORD_LOCKOUT_MINUTES: z.coerce.number().int().min(1).max(1440).default(15),
...
export const env = {
  ...
  passwordExpiryDays: parsed.data.PASSWORD_EXPIRY_DAYS,
  passwordHistoryCount: parsed.data.PASSWORD_HISTORY_COUNT,
  passwordMinAgeDays: parsed.data.PASSWORD_MIN_AGE_DAYS,
  passwordMaxFailedLogins: parsed.data.PASSWORD_MAX_FAILED_LOGINS,
  passwordLockoutMinutes: parsed.data.PASSWORD_LOCKOUT_MINUTES,
